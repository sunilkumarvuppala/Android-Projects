   ...
   // final ImageView star = (ImageView) activity.findViewById(R.id.imageStar);
    ...
    protected void onPostExecute(final ArrayList<App> apps) {
     ...
     //   Log.d("data",apps.toString());
        ...
        Firebase fire = new Firebase("https://firecastdemo.firebaseio.com/");
       // Log.d("something",mAuth.getCurrentUser().getUid());
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(!image_type){ //if image is grey (not present in db)
                   ...
                   // star.setImageResource(R.drawable.favoritesyellow);
                    ...
                    releaseDates.get(position).toString(), prices.get(position).toString(),categories.get(position),imgURL.get(position));*/
                    //add to database  //add id and img url
                    // Get a reference to our posts


                    //add to firebase
                    database = FirebaseDatabase.getInstance().getReference();
                    App newapp = new App(ids.get(position), appNames.get(position), categories.get(position),
                            releaseDates.get(position),prices.get(position),categories.get(position),imgURL.get(position));
                    database.child("users").child(MainActivity.name).child("apps").child(appNames.get(position)).setValue(newapp);
